...
public class ExceldataEOController extends BaseController<ExceldataEO>{
    ...
    public ResponseMessage delete(@PathVariable String carid) throws Exception {
        ...
        logger.info("delete from T_EXCELDATA where carid = {}", carid);
    ...
    */
    ...
    public void download(HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            List<ExceldataEODto> datasDto = new ArrayList<>();
            if(datas!=null && !datas.isEmpty()){
                for(ExceldataEO eo : datas){
                    CarQuoteDto dto = new CarQuoteDto();
                    BeanUtils.copyProperties(eo, dto);
                    datasDto.add(dto);
                }
            }
            workbook = ExcelExportUtil.exportExcel(exportParams, CarQuoteDto.class, datasDto);
            os = response.getOutputStream();
            workbook.write(os);
            os.flush();
        } catch (IOException e) {
